...
func (l *linux) addSudo(cmds []string) []string {
...
func (l *linux) getPermission(ctx context.Context, path string) (p permission, err error) {
	...
	cmd := shellescape.QuoteCommand(l.addSudo([]string{"stat", "-c", "%u %g %a", path}))
...
func (l *linux) mkdirp(ctx context.Context, path string) (err error) {
	cmd := shellescape.QuoteCommand(l.addSudo([]string{"mkdir", "-p", path}))
	return l.exec(ctx, &remote.Cmd{Command: cmd})
}

func (l *linux) cat(ctx context.Context, path string) (s string, err error) {
	stdout := new(bytes.Buffer)
	cmd := shellescape.QuoteCommand(l.addSudo([]string{"cat", path}))
...
func (l *linux) mv(ctx context.Context, old, new string) (err error) {
	cmd := shellescape.QuoteCommand(l.addSudo([]string{"mv", old, new}))
	return l.exec(ctx, &remote.Cmd{Command: cmd})
}
...
func (l *linux) remove(ctx context.Context, path, recyclePath string) (err error) {
	...
	if recyclePath != "" {
		...
		cmd = fmt.Sprintf(`{ [ ! -e %s ] || { %s && %s ;} ;}`,
			...
			shellescape.QuoteCommand(l.addSudo([]string{"mkdir", "-p", recycleFolder})),
			shellescape.QuoteCommand(l.addSudo([]string{"mv", path, recycleFolder})),
		)
	} else {
		cmd = fmt.Sprintf(`{ [ ! -e %s ] || %s ;}`,
			shellescape.Quote(path),
			shellescape.QuoteCommand(l.addSudo([]string{"rm", "-rf", path})),
		)
	}
	return l.exec(ctx, &remote.Cmd{Command: cmd})
}
